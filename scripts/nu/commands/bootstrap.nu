...
def main [] {
  ...
  if ($repo | path exists) {
    ...
    if ($env_file | path exists) {
      # Ensure encryption key exists in existing .env
      let env_content = (open $env_file --raw)
      if not ($env_content | str contains "MJ_BASE_ENCRYPTION_KEY") {
        let key = (^openssl rand -base64 32 | complete | get stdout | str trim)
        $"\n# Base encryption key (auto-generated by mjd bootstrap)\nMJ_BASE_ENCRYPTION_KEY='($key)'\n" | save -a $env_file
        info "Added MJ_BASE_ENCRYPTION_KEY to .env"
      }
      ...
      let key = (^openssl rand -base64 32 | complete | get stdout | str trim)
      let content = (open $env_file --raw | str replace "__AUTO_GENERATED__" $key)
